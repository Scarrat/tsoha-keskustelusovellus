...
def post_thread(topic, id, user):
    if topic:
        ...
        db.session.execute(
            "UPDATE cats SET subcats =subcats +1 where id=:id", {"id": id})
        db.session.commit()
        return True
    else:
        return False
...
def post_message(content, id, user, type):
    if content:
        ...
        result = db.session.execute(
            "SELECT cat_id FROM threads WHERE id=:id", {"id": id})
        theid = result.fetchone()[0]
        db.session.execute(
            "UPDATE cats SET messagecount =messagecount +1 where id=:id", {"id": theid})
        db.session.commit()
        return True
    else:
        return False
...
def deletet(id):
    ...
    db.session.execute(
        "UPDATE cats SET subcats =subcats -1 where id=:id", {"id": id1})
    result = db.session.execute(
...
def deletem(id):
    result = db.session.execute(
        "SELECT thread_id FROM messages WHERE id=:id", {"id": id})
    id1 = result.fetchone()[0]
    result = db.session.execute(
        "SELECT cat_id FROM threads WHERE id=:id", {"id": id1})
    id2 = result.fetchone()[0]
    db.session.execute(
        "UPDATE cats SET messagecount =messagecount -1 where id=:id", {"id": id2})
